...
export function SettingsProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchBranding = async () => {
    try {
      const res = await api.get("/admin/settings/public/branding");
      if (res.data.success) {
  ...
  const applyBranding = (data: Record<string, string>) => {
    ...
    // Update Title
    const platformTitle = data.platform_name || data.platform_operator_name || "MP-MLA Platform";
    document.title = `${platformTitle} - Master Dashboard`;
