  ...
  return (
    <TooltipProvider delayDuration={0}>
      <motion.aside
        ...
        className="h-screen bg-gradient-to-b from-[#1e1b4b] via-[#2d2a70] to-[#121240] text-white border-r border-white/10 flex flex-col fixed left-0 top-0 z-40 transition-all duration-300 shadow-2xl"
      >
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
          ...
          if (collapsed) {
            return (
              <Tooltip key={item.href}>
                ...
                <TooltipContent side="right" sideOffset={14} className="font-bold text-xs ml-1 bg-[#2d2a70] text-white border-white/10">
                  {item.label}
                </TooltipContent>
              </Tooltip>
            );
        ...
        {adminItems.length > 0 && (
          <>
            ...
            {adminItems.map((item) => {
              const isItemActive = item.href === "/subscriptions"
                ? location === "/subscriptions" || location.startsWith("/subscriptions/")
                : location === item.href;
              const content = (
                <div
                  className={cn(
                ...
                >
                  <item.icon
                    className={cn(
                      "h-5 w-5 min-w-5",
                      isItemActive ? "text-white" : "text-white/80 group-hover:text-white transition-colors",
                    )}
                  />
                  {!collapsed && (
        ...
        >
          ...
          {!collapsed && (
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 text-white/80 hover:bg-rose-500/20 hover:text-rose-350"
              onClick={() => logout()}
            >
