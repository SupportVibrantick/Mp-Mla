    ...
    if (email) {
      try {
        const emailSent = await sendEmailWithAttachment(
          tenantId,
          email,
          `Governance Report: ${type === "consolidated" ? "Consolidated Executive" : type.toUpperCase()}`,
          `<div style="font-family: sans-serif; padding: 20px; line-height: 1.6; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1);">
            <h2 style="color: #13538A; margin-top: 0;">Governance Report Generated</h2>
            <p>Dear recipient,</p>
            <p>Please find attached the official, multi-page PDF governance report for your review.</p>
            <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
              <tr style="background-color: #f8fafc;">
                <td style="padding: 10px; border: 1px solid #cbd5e1; font-weight: bold; width: 40%;">Report Type:</td>
                <td style="padding: 10px; border: 1px solid #cbd5e1;">${type === "consolidated" ? "Consolidated Executive Overview" : type.toUpperCase()}</td>
              </tr>
              <tr>
                <td style="padding: 10px; border: 1px solid #cbd5e1; font-weight: bold;">Generated By:</td>
                <td style="padding: 10px; border: 1px solid #cbd5e1;">${req.user?.name || req.user?.email || "Platform Admin"}</td>
              </tr>
              <tr style="background-color: #f8fafc;">
                <td style="padding: 10px; border: 1px solid #cbd5e1; font-weight: bold;">Reference Number:</td>
                <td style="padding: 10px; border: 1px solid #cbd5e1;">${referenceNumber}</td>
              </tr>
              ${dateRangeText ? `
              <tr>
                <td style="padding: 10px; border: 1px solid #cbd5e1; font-weight: bold;">Date Range:</td>
                <td style="padding: 10px; border: 1px solid #cbd5e1;">${dateRangeText}</td>
              </tr>` : ""}
            </table>
            <p style="font-size: 11px; color: #64748b; margin-top: 30px; border-top: 1px solid #e2e8f0; padding-top: 15px;">
              This email was sent automatically from the Constituency Administration Portal.
            </p>
          </div>`,
          [{ filename, content: pdfStream }]
        );

        if (emailSent) {
          res.json({ success: true, message: `Report emailed successfully to ${email}.` });
        } else {
          res.status(500).json({ success: false, error: "Failed to send email. Ensure SMTP settings are enabled and configured." });
        }
      } catch (error: any) {
        res.status(500).json({ success: false, error: error.message || "Failed to process PDF report email." });
      }
      return;
    }
